...
class PixiMovingCardPlugin implements JsPsychPlugin<Info> {
  ...
  async trial(display_element: HTMLElement, trial: TrialType<Info>) {
    ...
    // Color constants for feedback states (not from theme literal type)
    ...
    function moveRandomOption() {
      ...
      if (bestPos) {
        ...
        const animate = () => {
          ...
          opt.container.x = startX + (targetPos.x - startX) * ease;
          opt.container.y = startY + (targetPos.y - startY) * ease;
